...
export class DataLocal {
  static init() {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (
        !this.db.objectStoreNames.contains(GameConstant.INDEXEDDB_STORE_NAME)
      ) {
  ...
  static getCurrentLevel() {
    this.getData(GameConstant.INDEXEDDB_CURRENT_LEVEL_KEY)
      .then((value) => {
        if (typeof value === "undefined") {
          this.currentLevel = 1;
          this.addData(
            GameConstant.INDEXEDDB_CURRENT_LEVEL_KEY,
            this.currentLevel
          );
        } else {
  ...
  static getCurrency() {
    this.getData(GameConstant.INDEXEDDB_CURRENCY_KEY)
      .then((value) => {
        if (typeof value === "undefined") {
          this.currency = 0;
  ...
  static getStartNumber() {
    this.getData(GameConstant.INDEXEDDB_START_NUMBER_KEY)
      .then((value) => {
        if (typeof value === "undefined") {
          this.startNumber = GameConstant.PLAYER_START_NUMBER;
          this.addData(
            GameConstant.INDEXEDDB_START_NUMBER_KEY,
            this.startNumber
          );
        } else {
  ...
  static getUserIncome() {
    this.getData(GameConstant.INDEXEDDB_INCOME_KEY)
      .then((value) => {
        if (typeof value === "undefined") {
  ...
  static addData(key, value) {
    const userData = this.db
      .transaction(GameConstant.INDEXEDDB_STORE_NAME, "readwrite")
      .objectStore(GameConstant.INDEXEDDB_STORE_NAME);
  ...
  static updateCurrentLevelData(value) {
    ...
    request.onsuccess = (event) => {
      ...
      var requestUpdate = userData.put(
        data,
        GameConstant.INDEXEDDB_CURRENT_LEVEL_KEY
      );
