...
var allModel = read("animations");
allModel.forEach((model, index) => {
  fs.appendFileSync(OUTPUT_PATH, JSON.stringify(model) + ",\n");
  console.log("Writing animation: " + model.key + " - " + model.size.toFixed(2) + " KB");
  if (index === allModel.length - 1) {
    console.log("All animation passed: " + allModel.length);
    totalModelSize = allModel.reduce((total, model) => total + model.size, 0);
    console.log("Total animation size: " + totalModelSize.toFixed(2) + " KB" + "\n");
  }
});
...
totalSize = totalSpriteSize + totalTextureSize + totalFontSize + totalModelSize + totalSpineSize + totalAudioSize + totalVideoSize;
